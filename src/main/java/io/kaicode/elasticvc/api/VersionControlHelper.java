...
public class VersionControlHelper {
	...
	public BoolQuery.Builder getUpdatesOnBranchDuringRangeCriteria(String path, Date start, Date end) {
		...
		BoolQuery.Builder innerBool = bool()
				.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.START).gte(of(start.getTime())).lte(of(end.getTime())))))
				.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.END).gte(of(start.getTime())).lte(of(end.getTime())))));
		return bool()
			...
			if (!startBranchTimepoint.getHead().equals(endBranchTimepoint.getHead())) {
				...
				shouldsQuery.should(
						bool(b -> b
								...
								.must(bool(bq -> bq
										.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.START).gte(of(startTime)).lte(of(endTime)))))
										.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.END).gte(of(startTime)).lte(of(endTime)))))))));
			}
		}
	...
	private BranchCriteria getBranchCriteria(Branch branch, Date timepoint, Map<String, Set<String>> versionsReplaced, ContentSelection contentSelection, Commit commit, boolean skipRoot) {
		...
		switch (contentSelection) {
			case STANDARD_SELECTION -> {
				...
				thisBranchShouldClause.must(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.START).lte(of(timepoint.getTime())))));
				thisBranchShouldClause.must(bool(b -> b
						.should(bool(bq -> bq.mustNot(existsQuery(Entity.Fields.END))))
						.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.END).gt(of(timepoint.getTime())))))
				));
				...
				if (commit.isRebase()) {
					...
					for (BranchTimeRange branchTimeRange : branchTimeRanges) {
						...
						branchQueryBuilder.should(bool(b -> b
								...
								.must(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.START).gt(of(branchTimeRange.start().getTime())))))
								.must(bool(bq -> bq
												.should(bool(sb -> sb.mustNot(exists(eq -> eq.field(Entity.Fields.END)))))
												.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.END).lte(of(branchTimeRange.end().getTime())))))
										)
								)));
					}
				}
			...
			case UNPROMOTED_CHANGES_AND_DELETIONS_ON_THIS_BRANCH -> {
				...
				thisBranchShouldClause.must(bool(b -> b
						.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.START).gte(of(startPoint.getTime())))))
						.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.END).gte(of(startPoint.getTime())))))));
			...
			case UNPROMOTED_CHANGES_ON_THIS_BRANCH -> {
				...
				thisBranchShouldClause
						.must(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.START).gte(of(startPoint.getTime())))))
						.mustNot(existsQuery(Entity.Fields.END));
			}
		}
	...
	private Map<String, Set<String>> addParentCriteriaRecursively(BoolQuery.Builder branchCriteria, Branch branch, Map<String, Set<String>> versionsReplaced, boolean skipRoot) {
		...
		if (parentPath != null) {
			...
			branchCriteria.should(bool(b -> b
					...
					.must(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.START).lte(of(base.getTime())))))
					.must(bool(bq -> bq
							.should(bool(sb -> sb.mustNot(existsQuery(Entity.Fields.END))))
							.should(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.END).gt(of(base.getTime())))))))
			));
	...
	<T extends DomainEntity<?>> void endOldVersions(Commit commit, String idField, Class<T> entityClass, Collection<?> ids, ElasticsearchRepository<T, String> repository) {
		...
		final NativeQuery query = new NativeQueryBuilder()
				.withQuery(bool(b -> b
						...
						.must(range(rq -> rq.untyped(urq -> urq.field(Entity.Fields.START).lt(of(commit.getTimepoint().getTime())))))
